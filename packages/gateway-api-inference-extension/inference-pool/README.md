# Inference Pool package

Provides a [Kpt] package for deploying Inference Pool extension for Inference Gateway from templates in the upstream
Helm chart.

## Quick steps

1. Fetch package to local directory `my-model-pool`

   ```shell
   kpt pkg get --for-deployment https://github.com/memes/google-demo-helper-modules.git/packages/inference-pool my-model-pool
   ```
...
1. Deploy the package
   ...
   1. Render to YAML manifests
      ...
      ```shell
      kpt fn render my-model-pool -o path/to/contain/generated/manifests
      ```
   ...
   1. Deploy with [kpt]
      ...
      ```shell
      kpt live init my-model-pool
      kpt live apply my-model-pool
      ```
