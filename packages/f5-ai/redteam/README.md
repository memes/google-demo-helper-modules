# F5 AI Red Team package

Provides a [Kpt] package for deploying F5 AI Red Team components from manifests extracted from an embedded helm chart.

## Quick steps

1. Fetch package to local directory `f5-ai-redteam`

   ```shell
   kpt pkg get --for-deployment https://github.com/memes/google-demo-helper-modules.git/packages/f5-ai/redteam f5-ai-redteam
   ```
...
1. Deploy the package
   ...
   1. Render to YAML manifests
      ...
      ```shell
      kpt fn render f5-ai-redteam -o path/to/contain/generated/manifests
      ```
   ...
   1. Deploy with [kpt]
      ...
      ```shell
      kpt live init f5-ai-redteam
      kpt live apply f5-ai-redteam
      ```
...
1. Edit base settings
   ...
   1. Review [helm-chart.yaml]
      ...
      E.g. to change the `nodeSelector` value for `nvidia-gpu-a100-40gb` resource profile so that deployed models are
...
[Kptfile]: ./Kptfile
[secrets-sync-generator-cai-workflows-auth.yaml]: ./secrets-sync-generator-cai-workflows-auth.yaml
[secrets-sync-generator-prefect-server-auth.yaml]: ./secrets-sync-generator-prefect-server-auth.yaml
